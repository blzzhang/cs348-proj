...
$myfile = fopen("Schedule/courses.txt", "r") or die("Unable to open file!");

while(!feof($myfile)) {
  $term = trim(fgets($myfile));
  $sub = trim(fgets($myfile));
  $cat = trim(fgets($myfile));
  $lev = trim(fgets($myfile));
  parse_schedule($term, $sub, $cat, $lev);
}
fclose($myfile);
